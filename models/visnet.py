...
class EquivariantScalar(torch.nn.Module):
    ...
    Args:
        hidden_channels (int): The number of hidden channels in the node
            embeddings.
    """
    def __init__(self, hidden_channels: int, use_SFL=False) -> None:
        super().__init__()
...
class ViSNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.output_model = EquivariantScalar(hidden_channels=hidden_channels, use_SFL=self.use_SFL)
    ...
    def forward(self, batched_data) -> Tuple[Tensor, Optional[Tensor]]:
        z = batched_data['z']
