import pickle
from utils.utils import MolTransformFn, MolCollateFn, LBATransformFn, LBACollateFn, PPBCollateFn
import os
from utils.inmemory_dataset import InMemoryDataset

def get_data_loader(args, mode):

    if args.dataset in ['LBA_30', 'LBA_60']:    
        collate_fn = LBACollateFn(args)
        transform_fn = LBATransformFn(args)
    elif args.dataset == 'PPB':
        collate_fn = PPBCollateFn()
    else:
    ...
    elif mode in ['test', 'test_id', 'test_xood', 'test_yood', 'test_xyood']:
...
def get_data_loader_for_visualization(args, mode):
    ...
    if mode == 'train':
        ...
        train_dl = train_data.get_data_loader(batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn)
        valid_dl = valid_data.get_data_loader(batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn)
